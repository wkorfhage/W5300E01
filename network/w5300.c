...
#define W5300BASE_ADDRESS 0x10000000
...
void set_addresses(void)
{
        ...
        w5300_base = (unsigned short *) mmap((void *) 0, (size_t) 0x100,
                PROT_READ | PROT_WRITE, MAP_SHARED, memfd, W5300BASE_ADDRESS);
        printf ("mmap returned %X\n", (unsigned int) w5300_base);
        if (w5300_base == MAP_FAILED) {
	...
	UPORTR = MR + 0x4C/2;
	FMTUR = MR + 0x4E/2;
	Pn_BRDYR[0] = MR + 0x60/2;
	Pn_BDPTHR[0] = MR + 0x62/2;
	Pn_BRDYR[1] = MR + 0x64/2;
	Pn_BDPTHR[1] = MR + 0x66/2;
	Pn_BRDYR[2] = MR + 0x68/2;
	Pn_BDPTHR[2] = MR + 0x6A/2;
	Pn_BRDYR[3] = MR + 0x6C/2;
	Pn_BDPTHR[2] = MR + 0x6E/2;
	IDR = MR + 0xFE/2;
}
